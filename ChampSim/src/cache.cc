...
void CACHE::handle_fill()
{
  ...
  if (MSHR.next_fill_cycle <= current_core_cycle[fill_cpu])
  {
    ...
    if (do_fill)
    {
      ...
      //   // lower_level->return_data(&writeback_packet);

      //   if (cache_type == IS_L1D || cache_type == IS_L1I)
      //   {
      //     ooo_cpu[fill_cpu].L2C.return_data(&writeback_packet);
      //   }
      //   else if (cache_type == IS_L2C)
      //   {
      //     uncore.LLC.return_data(&writeback_packet);
      //   }
      // }
...
int CACHE::prefetch_line(uint64_t ip, uint64_t base_addr, uint64_t pf_addr, int pf_fill_level, uint32_t prefetch_metadata)
{
  ...
  if (PQ.occupancy < PQ.SIZE)
  {
    if ((base_addr >> LOG2_PAGE_SIZE) == (pf_addr >> LOG2_PAGE_SIZE))
    {
      ...
      uint32_t set = get_set(base_addr);
      uint32_t way = get_way(base_addr, set);
...
void CACHE::increment_MSHR_FULL(uint64_t address)
{
  MSHR.FULL++;
}