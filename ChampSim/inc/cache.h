...
// L1 DATA CACHE
#define L1D_SET 64 // 64
#define L1D_WAY 12 // 12
...
// LAST LEVEL CACHE
#define LLC_SET NUM_CPUS*1024 //32768 8192 2048 1024
#define LLC_WAY 32
...
class CACHE : public MEMORY {
  public:
    ...
    CACHE(string v1, uint32_t v2, int v3, uint32_t v4, uint32_t v5, uint32_t v6, uint32_t v7, uint32_t v8) 
		...
		prefetch_bit = new uint8_t* [NUM_SET];
        for (uint32_t i=0; i<NUM_SET; i++) {
            block[i] = new BLOCK[NUM_WAY]; 
			prefetch_bit[i] = new uint8_t[NUM_WAY];
    ...
    void return_data(PACKET *packet),
         ...
         increment_MSHR_FULL(uint64_t address);
